 ...
 *     </ul>
 *     When either dimension is exhausted, {@link #acquire(FileDownloadTask)} blocks the submitting (main)
 ...
 *           перед загрузкой и освобождает её в терминале. Размер = {@code min(config, Xmx - 100МБ)} (где
 ...
 *     </ul>
 *     Когда любое измерение исчерпано, {@link #acquire(FileDownloadTask)} блокирует отправляющий (главный)
...
public final class InFlightBudget
{
     ...
     **/
    public void acquire(FileDownloadTask task)
    {
     ...
     *     раз, в терминале задачи, чтобы освободившаяся ёмкость впустила новую работу. <br>
     * ==================================================================<br>
     * EN: @param task the task reaching a terminal state / RU: @param task задача, достигшая терминала <br>
     **/
    public void release(FileDownloadTask task)
    {
    ...
    /**
     * EN: Estimates a task's in-flight memory footprint in MB (compressed + decompressed; for a multi-part
     *     file the sum of all parts, since they are held together), using {@code DEFAULT_FILE_MB} when the
     *     lengths are unknown, and clamped to the total budget so an oversized file runs alone without ever
     *     requesting more permits than exist. <br>
     * RU: Оценивает потребление памяти задачи в МБ (сжатое + распакованное; для многочастного файла — сумма
     *     всех частей, так как они держатся вместе), используя {@code DEFAULT_FILE_MB} при неизвестных
     *     длинах, и ограничивая общим бюджетом, чтобы большой файл качался в одиночку и не просил больше
     *     квот, чем существует. <br>
     * ==================================================================<br>
     ...
     **/
    private int estimateMB(FileDownloadTask task)
    {
